...
#var_gnss = np.array(([0.01, 0.01, 0.01]))
#var_lidar = np.array(([1.44,1.15,0.58]))
...
for k in range(1, imu_f.data.shape[0]):  # start at 1 b/c we have initial prediction from gt
    ...
    quat = Quaternion(q_est[k-1, 0], q_est[k-1, 1], q_est[k-1, 2], q_est[k-1, 3])
    Cns = quat.to_mat()
    ...
    q_check = Quaternion(euler=q_euler).quat_mult_right(q_est[k-1]) #updating q

    # 1.1 Linearize the motion model and compute Jacobians
    F = np.identity(9)
    F[0:3,3:6] = delta_t*np.eye(3)
    doq = skew_symmetric(Cns.dot(imu_f.data[k-1]))
    doq = -1*delta_t*doq
    F[3:6,6:9] = doq
    ...
    Q[3:6,3:6] = ((delta_t**2)*(var_imu_w))*np.eye(3)

    temp1 = F.dot(p_cov[k-1].dot(F.T))
    temp2 = l_jac.dot(Q.dot(l_jac.T))
    p_cov_check = np.add(temp1, temp2)

    # 3. Check availability of GNSS and LIDAR measurements
    if(gnss_i < gnss.t.shape[0] and gnss.t[gnss_i] >= imu_f.t[k-1] and gnss.t[gnss_i] < imu_f.t[k]):
        #print('Using GNSS')
        p_check, v_check, q_check, p_cov_check = measurement_update(var_gnss, p_cov_check, gnss.data[gnss_i], p_check, v_check, q_check)
        gnss_i = gnss_i+1 #updating gnss index

    if(lidar_i < lidar.t.shape[0] and lidar.t[lidar_i] >= imu_f.t[k-1] and lidar.t[lidar_i] < imu_f.t[k]):
        #print('Using Lidar')
        p_check, v_check, q_check, p_cov_check = measurement_update(var_lidar, p_cov_check, lidar.data[lidar_i], p_check, v_check, q_check)
        lidar_i = lidar_i+1 #updating lidar index
